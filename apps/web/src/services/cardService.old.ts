...
export const cardService = {  /**
   * Create a new card using the cloud functions API
   */
  async createCard(cardData: Omit<Card, 'id' | 'createdAt' | 'updatedAt' | 'active'>): Promise<{ cardId: string }> {
    ...
    try {
      // Import cloudFunctionsService dynamically to avoid circular dependencies
      const { default: cloudFunctionsService } = await import('./cloudFunctionsService');
      
      console.log('Calling createCard cloud function via service');
      const result = await cloudFunctionsService.createCard(cardData);
      
      console.log('Card created successfully via cloud function:', result.cardId);
      return { cardId: result.cardId };
    } catch (error: any) {
      console.error('Error creating card via cloud function:', error);
